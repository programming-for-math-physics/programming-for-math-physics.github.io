...
# {C.section}-{C.inc_subsection} リスト式
...
# {C.section}-{C.inc_subsection} リストに対する操作

__要点:__

* len関数 : `len(リスト)` でリストの長さ
* appendメソッド : `リスト.append(要素)` でリスト末尾に要素を追加
* 添字式 : `リスト[i]` でリストのi番目の要素をアクセス
...
# {C.section}-{C.inc_section} for文によるリストの作成
...
# {C.section}-{C.inc_section} for文によるリストの走査
...
for i in range(a, b):
    ...
    ...
```
で i = a, a+1, a+2, ..., b-1 として文(...)を実行するものとして理解して
...
# {C.section}-{C.inc_subsection} (参考) リストの内包表記
