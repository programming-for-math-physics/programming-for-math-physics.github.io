...
# {C.inc_section} : matplotlibのimport (おまじない)
...
# {C.section}-{C.inc_subsection} : 1次元(x軸とy軸)のプロット

* $y = x^2$のグラフとか, $y=\sin x$のグラフとか, 1変数関数のグラフを表示できる
* 基本
 - X = x座標だけを並べたリストまたは配列
 - Y = y座標だけを並べたリストまたは配列を用意する
 - plt.plot(x座標だけを並べたリストまたは配列, y座標だけを並べたリストまたは配列)
...
# {C.section}-{C.inc_subsection} : 少し違うフレーバーの描画
...
# {C.section}-{C.inc_subsection} : ややこしい計算結果の描画
